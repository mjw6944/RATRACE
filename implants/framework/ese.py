...
class ESENT_PAGE:
    def __init__(self, db, data=None):
        ...
        self.record = None
        if data is not None:
            self.record = ESENT_PAGE_HEADER(self.__DBHeader['Version'], self.__DBHeader['FileFormatRevision'],
                                            self.__DBHeader['PageSize'], data)
    ...
    def dump(self):
        ...
        for i in range(self.record['FirstAvailablePageTag']):
            tag = tags[-4:]
            if self.__DBHeader['Version'] == 0x620 and self.__DBHeader['FileFormatRevision'] > 11 and self.__DBHeader[
                'PageSize'] > 8192:
    ...
    def getTag(self, tagNum):
        ...
        if self.__DBHeader['Version'] == 0x620 and self.__DBHeader['FileFormatRevision'] >= 17 and self.__DBHeader[
            'PageSize'] > 8192:
            valueSize = unpack('<H', tag[:2])[0] & 0x7fff
            valueOffset = unpack('<H', tag[2:])[0] & 0x7fff
...
class ESENT_DB:
    ...
    def mountDB(self):
        ...
        LOG.debug("Database Version:0x%x, Revision:0x%x" % (self.__DBHeader['Version'],
    ...
    def printCatalog(self):
        ...
        for table in list(self.__tables.keys()):
            ...
            for column in list(self.__tables[table]['Columns'].keys()):
                ...
                print("%s%-5d%-30s%s" % (indent * 2, record['Identifier'], column.decode('utf-8'),
        ...
        for column in list(columns.keys()):
            ...
            if columnRecord['Identifier'] <= dataDefinitionHeader['LastFixedSize']:
                ...
                if taggedItemsParsed is False and (variableDataBytesProcessed + variableSizeOffset) < tagLen:
                    ...
                    firstOffsetTag = (unpack('<H', tag[index + 2:][:2])[
                                          0] & 0x3fff) + variableDataBytesProcessed + variableSizeOffset
                    while True:
                        ...
                        if self.__DBHeader['Version'] == 0x620 and self.__DBHeader['FileFormatRevision'] >= 17 and \
                                self.__DBHeader['PageSize'] > 8192:
                            flagsPresent = 1
                        else:
                ...
                if columnRecord['Identifier'] in taggedItems:
                    offsetItem = variableDataBytesProcessed + variableSizeOffset + \
