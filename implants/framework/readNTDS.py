"""
I STOLE THIS ALL FROM IMPACKET
...
class NTDSHashes:
    ...
    def __init__(self, ntdsFile, bootKey, isRemote=False, history=False, noLMHash=True, remoteOps=None,
                 useVSSMethod=False, remoteSSMethodWMINTDS=False, justNTLM=False, pwdLastSet=False, resumeSession=None,
                 outputFileName=None,
    ...
    def __getPek(self):
        ...
        while True:
            ...
            if record is None:
                ...
                break
            elif record[self.NAME_TO_INTERNAL['sAMAccountType']] in self.ACCOUNT_TYPES and record[
                self.NAME_TO_INTERNAL['instanceType']] & 4:  # "The object is writable on this directory":
                ...
                if userProperty['PropertyName'].decode('utf-16le') == 'Primary:Kerberos-Newer-Keys':
                    ...
                    for credential in range(kerbStoredCredentialNew['CredentialCount']):
                        ...
                        if keyDataNew['KeyType'] in self.KERBEROS_TYPE:
                            answer = "%s:%s:%s" % (userName, self.KERBEROS_TYPE[keyDataNew['KeyType']],
                                                   hexlify(keyValue).decode('utf-8'))
                        else:
                            answer = "%s:%s:%s" % (userName, hex(keyDataNew['KeyType']),
                                                   hexlify(keyValue).decode('utf-8'))
                ...
                if attId == LOOKUP_TABLE['dBCSPwd']:
                    ...
                    if attr['AttrVal']['valCount'] > 0:
                        try:
                            ...
                            LOG.error('Cannot get sAMAccountName for %s' % record['pmsgOut'][replyVersion]['pNC'][
                                'StringName'][:-1])
                            userName = 'unknown'
                    else:
                        LOG.error(
                ...
                try:
                    if self.__remoteOps is not None:
                        try:
                            ...
                            if os.getenv('KRB5CCNAME') is not None and (
                                    self.__justUser is not None or self.__ldapFilter is not None):
                    ...
                    while True:
                        try:
                            record = self.__ESEDB.getNextRow(self.__cursor,
                                                             filter_tables=self.__filter_tables_usersecret)
                        except:
                            LOG.error('Error while calling getNextRow(), trying the next one')
                            continue
                        ...
                        if record is None:
                            break
                        try:
                            if record[self.NAME_TO_INTERNAL['sAMAccountType']] in self.ACCOUNT_TYPES and record[
                                self.NAME_TO_INTERNAL[
                                    'instanceType']] & 4:  # "The object is writable on this directory"
                                self.__decryptHash(record, outputFile=hashesOutputFile)
                                if self.__justNTLM is False:
                                    self.__decryptSupplementalInfo(record, None, keysOutputFile, clearTextOutputFile)
                        except Exception as e:
