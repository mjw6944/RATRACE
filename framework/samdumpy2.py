"""
Originally from https://github.com/vincd/samdumpy/blob/master/samdum.py

Credit to Vincent D. I thank you for your hard work.

Modified to suit the needs of mimikitty
"""
...
def get_hashes(h, sam_key):
    ...
    for regkeyname in h.regEnumKey(n):
        ...
        lm_hash = get_hash(sam_key, user_account, rid, 13, almpassword) or empty_lm
        nt_hash = get_hash(sam_key, user_account, rid, 14, antpassword) or empty_nt

        # Print in the requested format: Username:LMhash:NThash:
        print(f"{username}:{lm_hash}:{nt_hash}:")
